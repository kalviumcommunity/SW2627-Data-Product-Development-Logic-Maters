...
def render(filtered: pd.DataFrame, full: pd.DataFrame) -> None:
    ...
    st.plotly_chart(
        bar_chart(metrics, "route", "delay_rate",
                  "Observed delay rate by route", y_label="Delay rate (%)"),
        width="stretch",
    )
    st.subheader("Route detail")
    choice = st.selectbox("Route", metrics["route"].tolist())
    ...
    if route_trend.empty:
        ...
        return
    st.plotly_chart(
        trend_line(route_trend, y_column="shipments",
