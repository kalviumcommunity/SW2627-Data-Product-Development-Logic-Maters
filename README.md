...
Route cascade prediction baseline implemented (leakage-free features, chronological split, NumPy-only logistic regression, JSON artifacts, inference API, pipeline/dashboard/report integration with honest unavailable states); validated on showcase synthetic data, correctly unavailable on the LaDe window (94 delayed, 1 cascade).
...
- Stages: source build → ingestion → validation → **gate** → cleaning → integration → analytics → SQL cross-check → cascade → route risk → prediction → alerts → manifest
...
- Route cascade prediction baseline (`prediction_features.py`, `prediction_model.py`, `predict.py`, thresholds in `config/prediction_config.py`): validated, explainable ML baseline trained ONLY when the data meets minimum requirements (see Design.md §18.2). Target `cascade_flag ∈ {0,1}` reuses the existing cascade definition; population is delayed shipments at the initial-delay prediction point; features are prediction-time only (route, initial duration/sequence/stage/warehouse, timestamp parts, train-only route history) with an enforced leakage guard. Chronological train/test split; NumPy-only logistic regression; accuracy + precision/recall/F1 + ROC-AUC + PR-AUC + confusion matrix vs a majority baseline. JSON artifacts in `data/processed/models/` (gitignored). Showcase synthetic data trains successfully (test ROC-AUC ~0.77); the LaDe window (94 delayed, 1 cascade) correctly yields `PREDICTION_UNAVAILABLE`
...
- Pages: Overview (KPI cards, delay trend, route summary), Dataset explorer (structure, samples, missingness), Delay analysis (distribution, reasons, trends, segments), Routes (factual metrics, detail, trends, **route cascade risk with rate/recurrence charts, period trend, stage transitions**), Warehouses (metrics, transfer activity, trends), Cascades (candidates, depth, route/warehouse patterns, shipment journey inspector), Prediction (validated baseline status, test metrics, per-route estimated probabilities beside historical rates, single-shipment estimator; honest unavailable state when no model exists), Alerts (threshold-breach summary, alert table, per-alert evidence)
...
- ML baseline block: when the run trained a validated model, the report shows model status/version, train/test periods, test metrics, and per-route estimated probabilities (recomputed deterministically over the run's own output); otherwise it states "ML prediction was not generated because the available dataset did not meet the minimum validation requirements." Empirical historical risk is never labeled as ML prediction
